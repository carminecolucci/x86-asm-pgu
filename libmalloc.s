# Position independent code of Memory allocator
...
malloc_init:
	...
	int	$SYSCALL
	incl	%eax
	movl	%eax, current_break@GOTOFF(%edi)
	movl	%eax, heap_begin@GOTOFF(%edi)		# heap memory starts right after the break
	ret
...
malloc:
	...
	cmpl	$0, heap_begin@GOTOFF(%edi)		# heap is 0 before being initialized
	jne	malloc_start
	call	malloc_init

malloc_start:
	movl	8(%ebp), %ecx				# ecx: size
	movl	heap_begin@GOTOFF(%edi), %eax
	movl	current_break@GOTOFF(%edi), %ebx
...
do_break:
	...
	movl	%ebx, current_break@GOTOFF(%edi)	# save new break
